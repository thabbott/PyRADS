...
Ta_grid = np.arange(170.,370.1,20.)
RH_grid = np.arange(0.,1.1,0.1)

filename = 'output.compute_olr_lut.txt'
...
if saveOutput:
    ...
    f.write("\n")
    f.write("\n")
    f.write("Ts [K],\tTa [K],\tqcol [kg/m2],\tps [bar],\tslw [W/m2],\tolr [W/m2],\tsurface contribution to olr [W/m2],\tTransmission int[T dBdT(Ts) dn]/int[ dBdT(Ts) dn],\tSimple feedback model int[dB/dT*T]")
    f.write("\n")
    ...
    for Ts in Ts_grid:
        for Ta in Ta_grid:
            for RH in RH_grid:
                ...
                # setup grid:
                g = pyrads.SetupGrids.make_grid( Ta,Tstrat,N_press,wavenr_min,wavenr_max,dwavenr,params, RH=params.RH )
                ...
                # Compute surface longwave
                slw_spec = pyrads.Get_Fluxes.Fminus(g.Np-1,g)
                slw = simps(slw_spec,g.n)
                ...
                # Column water vapor
                qcol = trapz(g.q, x = g.p)/params.g
                print( "\n",Ts,Ta,qcol,g.ps/1e5,slw,olr,surf, "\n")

                f.write("%.2f,\t%.2f,\t%.8f,\t%.4f,\t%.8f,\t%.8f,\t%.8f,\t%.8f,\t%.8f" % (Ts,Ta,qcol,g.ps/1e5,slw,olr,surf,trans,lam) )
                f.write("\n")
